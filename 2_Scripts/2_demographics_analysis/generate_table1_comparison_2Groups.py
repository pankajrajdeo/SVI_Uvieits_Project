...
def parse_and_group_by_dx(df, dx_col, jia_pattern, uveitis_patterns):
    """Parses DX codes and groups rows into JIA, Any Uveitis, or Other."""
    ...
    def check_codes(dx_string):
        ...
        if has_uveitis: # Covers JIA-U and Uveitis-Only
            return 'Any Uveitis'
        elif has_jia: # Covers JIA-Only (since has_uveitis would have been caught above)
            return 'JIA'
        else:
            return 'Other'
    ...
    df_filtered = df[df['diagnosis_group'].isin(['JIA', 'Any Uveitis'])].copy()
    print(f"Filtered to JIA and Any Uveitis groups. Shape: {df_filtered.shape}")
    return df_filtered
...
def calculate_categorical_stats(df, group_col, value_col, metric_name):
    ...
    groups_with_data = sum([n_g1 > 0, n_g2 > 0])
    if groups_with_data <= 1:
        p_value_str = 'N/A (insufficient data for comparison)'
        print("(Note: Statistical testing skipped - data available for only one group, or table dimensions too small for test)")
    else:
        try:
    ...
    # Print results
    print(f"{'Category':<45} {group1_name:<20} {group2_name:<20} P-value")
    print(f"{'':<45} {'(N=' + str(n_g1) + ')':<20} {'(N=' + str(n_g2) + ')':<20} {p_value_str}")

    # Calculate and display percentages
    for category in contingency_table.columns:
        g1_count = contingency_table.loc[group1_name, category] if group1_name in contingency_table.index and category in contingency_table.columns else 0
        g2_count = contingency_table.loc[group2_name, category] if group2_name in contingency_table.index and category in contingency_table.columns else 0
        ...
        print(f"  {category:<43} {g1_str:<20} {g2_str:<20}")
    ...
    if metric_name not in ['ESR Status (Interpreted)', 'Vitamin D Status (Interpreted)']:
        ...
        if missing_g1 > 0 or missing_g2 > 0:
            print(f"  {'Missing/Unknown':<43} {missing_g1:<20} {missing_g2:<20}")
...
def calculate_continuous_stats(df, group_col, value_col, metric_name):
    ...
    if n_g1 > 0 and n_g2 > 0:
        try:
            # Ensure there's variance in both groups for Mann-Whitney U, or they are identical
            if group1_data.nunique() > 0 and group2_data.nunique() > 0 : # Check if nunique is at least 1
                 # If one group has no variance (all same values), but the other does, MWU can run
                 # If both groups have no variance AND values are different, MWU might raise error or give p=0/1
                 # If both groups have no variance AND values are same, p should be 1
                if group1_data.nunique() == 1 and group2_data.nunique() == 1 and group1_data.iloc[0] == group2_data.iloc[0]:
                    p_value_str = '1.0000'
                    print("(Both groups have identical constant values, setting p=1.0000)")
                else:
                    stat, p = stats.mannwhitneyu(group1_data, group2_data, alternative='two-sided', nan_policy='propagate')
                    p_value_str = f"{p:.4f}"
                    if p < 0.0001: p_value_str = "<0.0001"
            elif (n_g1 > 0 and n_g2 > 0) and (group1_data.nunique() == 0 or group2_data.nunique() == 0) : # Catches empty groups post-dropna if not caught by n_g1/n_g2
                 p_value_str = 'N/A (no data/variance in one/both groups after NaN removal)'
                 print("(Skipping p-value: No data or no variance in one or both groups after NaN removal for Mann-Whitney U test.)")
            else: # Fallback for other unhandled scenarios, though n_g1>0 and n_g2>0 should cover most
                 p_value_str = 'N/A (cannot compute)'
                 print("(Skipping p-value: Mann-Whitney U cannot be computed reliably with current data.)")


        except ValueError as e: # This might catch issues if one group is all NaNs or other specific edge cases
            print(f"(Mann-Whitney U error: {e}. Checking means if available...)")
            if not np.isnan(mean_g1) and not np.isnan(mean_g2) and mean_g1 == mean_g2: # MODIFIED
                 p_value_str = '1.0000'
                 print("(Means are identical, setting p=1)")
            else:
                 p_value_str = 'Error (ValueError)'
    else:
        print("(Skipping p-value: Not enough data in one or both groups for Mann-Whitney U test)") # MODIFIED

    # Format results
    g1_str = f"{mean_g1:.1f} ± {std_g1:.1f}" if n_g1 > 0 and not np.isnan(mean_g1) else "N/A"
    g2_str = f"{mean_g2:.1f} ± {std_g2:.1f}" if n_g2 > 0 and not np.isnan(mean_g2) else "N/A"
    if n_g1 == 1 and not np.isnan(mean_g1): g1_str = f"{mean_g1:.1f}"
    if n_g2 == 1 and not np.isnan(mean_g2): g2_str = f"{mean_g2:.1f}"

    # Print formatted results
    print(f"{'Value':<35} {group1_name:<20} {group2_name:<20} P-value")
    print(f"{'':<35} {'(N=' + str(n_g1) + ')':<20} {'(N=' + str(n_g2) + ')':<20} {p_value_str}")
    print(f"  Mean ± SD                 {g1_str:<20} {g2_str:<20}")

    # Report missing count based on total group size
    missing_g1 = total_group1 - n_g1
    missing_g2 = total_group2 - n_g2
    if missing_g1 > 0 or missing_g2 > 0:
         print(f"  {'Missing/Non-Numeric':<33} {missing_g1:<20} {missing_g2:<20}")


def combine_race_ethnicity(df, race_col, ethnicity_col):
    """Combines race and ethnicity into specified final categories."""
    ...
    if race_col not in df.columns or ethnicity_col not in df.columns:
        ...
        df['race_ethnicity_combined'] = 'Other/Multiethnic' # Default if columns are missing
        return df

    # Prepare series, ensuring copy, fillna, string type, lowercase, strip
    race_series_clean = df[race_col].copy().fillna('unknown').astype(str).str.lower().str.strip()
    ethnicity_series_clean = df[ethnicity_col].copy().fillna('unknown').astype(str).str.lower().str.strip()

    # Define standardization maps
    race_map = {
        'black or african american': 'Black',
        'white': 'White',
        'asian': 'Asian', # Keep Asian for intermediate step; will be grouped into Other/Multiethnic if not Black/White
        'american indian or alaska native': 'Other/Multiethnic',
        'native hawaiian or other pacific islander': 'Other/Multiethnic',
        'middle eastern': 'White', # Map to White as per existing convention in script
        'more than one race': 'Other/Multiethnic',
        'other': 'Other/Multiethnic',
        # Handle various forms of unknown/refused for race, map to a distinct "Unknown Race"
        'patient refused': 'Unknown Race',
        'please update': 'Unknown Race',
        'preferred category not available': 'Unknown Race',
        'no parent/primary caregiver present': 'Unknown Race',
        'unknown': 'Unknown Race', # Handle string 'nan'
        'nan': 'Unknown Race',    # Handle empty strings
    }
    # Apply race mapping. Values not in map become 'Other/Multiethnic' by default.
    standardized_race = race_series_clean.map(race_map).fillna('Other/Multiethnic')
    ...
    ethnicity_map = {
        ...
        # Handle various forms of unknown/refused for ethnicity, map to "Unknown Ethnicity"
        'patient refused': 'Unknown Ethnicity',
        'unknown': 'Unknown Ethnicity',
        'missing': 'Unknown Ethnicity',
        'nan': 'Unknown Ethnicity', # Handle string 'nan'
        '': 'Unknown Ethnicity',    # Handle empty strings
    }
    # Apply ethnicity mapping. Values not in map become 'Unknown Ethnicity' by default.
    standardized_ethnicity = ethnicity_series_clean.map(ethnicity_map).fillna('Unknown Ethnicity')

    # Initialize the combined column with the most general category
    df['race_ethnicity_combined'] = 'Other/Multiethnic'

    # Define boolean conditions for clarity
    is_black = (standardized_race == 'Black')
    is_white = (standardized_race == 'White')
    # Standardized races 'Asian', 'Other/Multiethnic', 'Unknown Race' will default to 'Other/Multiethnic' combined

    is_hispanic = (standardized_ethnicity == 'Hispanic or Latino')
    is_not_hispanic = (standardized_ethnicity == 'Not Hispanic or Latino')
    # Standardized ethnicity 'Unknown Ethnicity' will default to 'Other/Multiethnic' combined

    # Hispanic Latino groups
    df.loc[is_black & is_hispanic, 'race_ethnicity_combined'] = 'Black, Hispanic or Latino'
    df.loc[is_white & is_hispanic, 'race_ethnicity_combined'] = 'White, Hispanic or Latino'
    # If race is Asian/Other/Unknown and ethnicity is Hispanic, it remains 'Other/Multiethnic' (default)

    # Not Hispanic Latino groups
    df.loc[is_black & is_not_hispanic, 'race_ethnicity_combined'] = 'Black, Not Hispanic or Latino'
    df.loc[is_white & is_not_hispanic, 'race_ethnicity_combined'] = 'White, Not Hispanic or Latino'
    # If race is Asian/Other/Unknown and ethnicity is Not Hispanic, it remains 'Other/Multiethnic' (default)

    # If ethnicity is 'Unknown Ethnicity', it remains 'Other/Multiethnic' (default) regardless of race.
    # This logic correctly ensures that only specific Black/White combinations with known Hispanic/Not Hispanic status
    # get assigned, and everything else (including all 'Asian' regardless of ethnicity, all 'Unknown Race'
    # regardless of ethnicity, and all 'Unknown Ethnicity' regardless of race) becomes 'Other/Multiethnic'.
...
def main():
    ...
    OUTPUT_FILE_PATH = os.path.join(os.path.expanduser("~/Desktop"), "table1_comparison_output.txt")
    GROUPS_OUTPUT_FILE = os.path.join(os.path.expanduser("~/Desktop"), "patient_groups.csv")
    ...
    with open(OUTPUT_FILE_PATH, 'w') as f:
        ...
        print("      JIA and Any Uveitis groups with valid, non-missing data for that specific variable.")
        ...
        try:
            ...
            if df_analysis is None or df_analysis.empty:
                print("Error: Could not define JIA and Any Uveitis groups. Exiting.")
                sys.stdout = original_stdout 
                print("Error occurred during grouping, check table1_comparison_output.txt for details.")
                return 
            
            if len(df_analysis['diagnosis_group'].unique()) < 2:
                print("Error: Less than two groups found after filtering (expected JIA, Any Uveitis). Cannot perform comparison. Exiting.")
                sys.stdout = original_stdout
                print("Error: Less than two groups found after filtering, check table1_comparison_output.txt for details.")
                return

            print("\n========= Table 1: JIA vs Any Uveitis Comparison =========")
